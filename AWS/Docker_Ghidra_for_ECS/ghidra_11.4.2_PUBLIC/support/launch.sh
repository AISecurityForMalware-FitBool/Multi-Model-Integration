...
if [ ! $? -eq 0 ]; then
   # Lambda/컨테이너 환경에서는 TTY가 없으므로, 그냥 JAVA_HOME 강제 사용
   if [ -n "${JAVA_HOME}" ]; then
      echo "[INFO] No TTY detected. Forcing JAVA_HOME=${JAVA_HOME}"
      LS_JAVA_HOME="${JAVA_HOME}"
   else
      echo "[WARNING] JAVA_HOME not set. Falling back to PATH java"
      LS_JAVA_HOME="$(dirname $(dirname $(readlink -f $(which java))))"
   fi
fi
